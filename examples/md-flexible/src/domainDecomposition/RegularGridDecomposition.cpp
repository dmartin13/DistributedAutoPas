...
void RegularGridDecomposition::initializeMPICommunicator() {
  ...
  // Keep rank ordering stable during the transitional DistributedAutoPas integration.
  // DistributedAutoPas currently decomposes MPI_COMM_WORLD directly along x.
  autopas::AutoPas_MPI_Cart_create(AUTOPAS_MPI_COMM_WORLD, _dimensionCount, _decomposition.data(),
                                   const_cast<int *>(_periods.data()), false, &_communicator);
